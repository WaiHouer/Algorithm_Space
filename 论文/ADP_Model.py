...
def adp_model(K, S, E, A, U, b_last, N, sigma_hat, beta_e, beta_a, beta_u, eta, b_hat, lambda_b, C, lambda_c
    ...
    constant_e_k, constant_a_k, constant_u_k, coefficient_b_k, coefficient_c_k \
        = coefficient_calculate_k(K, S, E, A, U, N, sigma_hat, beta_e, beta_a, beta_u, eta)

    for o in range(0, K):
        model.addConstr(h_nxt[o] == S[o] - (constant_e_k[o] + constant_a_k[o] + constant_u_k[o]
                                            - gp.quicksum(b[j] * coefficient_b_k[o][j] for j in range(K))
                                            + gp.quicksum(c[j] * coefficient_c_k[o][j] for j in range(K))))
    for o in range(K, 2 * K):
        model.addConstr(h_nxt[o] == E[o - K] + (constant_e_k[o - K] + constant_a_k[o - K] + constant_u_k[o - K]
                                                - gp.quicksum(b[j] * coefficient_b_k[o - K][j] for j in range(K))
                                                + gp.quicksum(c[j] * coefficient_c_k[o - K][j] for j in range(K)))
                        - alpha[o - K] * E[o - K])
...
@njit()
def coefficient_calculate_k(K, S, E, A, U, N, sigma_hat, beta_e, beta_a, beta_u, eta):
    constant_e, constant_a, constant_u = np.zeros(K), np.zeros(K), np.zeros(K)  # 计算三个常数项
    for k in range(K):
        tem_e, tem_a, tem_u = 0, 0, 0
        for j in range(K):
            tem_e += E[j] / N[j] * beta_e[j] * sigma_hat[k][j]
            tem_a += A[j] / N[j] * beta_a[j] * sigma_hat[k][j]
            tem_u += U[j] / N[j] * beta_u[j] * sigma_hat[k][j]
        constant_e[k] = S[k] * tem_e
        constant_a[k] = S[k] * tem_a
        constant_u[k] = S[k] * tem_u

    coefficient_b, coefficient_c = np.zeros((K, K)), np.zeros((K, K))  # 计算决策变量的系数
    for k in range(K):
        for j in range(K):
            coefficient_b[k][j] = eta / N[j] * beta_u[j] * S[k] * sigma_hat[k][j]
            coefficient_c[k][j] = A[j] / N[j] / N[j] * (beta_u[j] - beta_a[j]) * S[k] * sigma_hat[k][j]

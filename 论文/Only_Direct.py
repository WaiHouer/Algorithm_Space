"""
整体复制了 Epidemic_Model_Final，只用于画出9月1日的直接预测部分
"""
import numpy as np
...
class Epidemic_Model:  # 完整传染病模型
    def __init__(self,file_name):
        ...
        self.actual = [[] for i in range(self.region_num)]  # 真实感染人数
        for i in range(self.region_num):
    ...
    def picture(self):
        ...
        plt.plot(t_range, total_I, color='darkred',
                 label='Fitting Number of Infected Individuals')
        ...
        plt.plot(t_actual_range, total_actual, color='orange', marker='.',
                 label='Actual Number of Infected Individuals')
        ...
        plt.plot(t_pre_range, total_I_pre_direct, color='salmon',
                 label=f'Predicted Number of Infected Individuals')  # 画出直接预测结果

        # month_num = [0, 30, 61, 91, 122, 153, 183, 214, 244, 275, 306]  # 画出年月日坐标（用于21-1-15训练集）

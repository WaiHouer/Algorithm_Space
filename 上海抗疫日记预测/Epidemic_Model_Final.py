...
class Epidemic_Model:  # 完整传染病模型
    def __init__(self,file_name):
        ...
        self.start = 0  # 开始时间点
        self.end = 44  # 结束时间点（20.4.13-21.1.13，此处为275）
        self.t_num = self.end - self.start + 1  # 时间长度

        self.fitting_num = 15  # 拟合小周期

        self.predict_num = 150  # 预测未来天数
    ...
    def picture(self):
        ...
        # plt.plot(t_range, total_R, label='Total_R', color='darkblue')  # 上海不要这个
        ...
        plt.plot(t_pre_range, self.E_pre_direct[0])  # 画出直接预测结果(上海外不用
        plt.plot(t_pre_range, self.A_pre_direct[0])  # 画出直接预测结果(上海外不用
        plt.plot(t_pre_range, self.Q_pre_direct[0])  # 画出直接预测结果(上海外不用
        plt.plot(t_pre_range, self.U_pre_direct[0])  # 画出直接预测结果(上海外不用
        # plt.plot(t_pre_range, self.R_pre_direct[0])  # 画出直接预测结果(上海外不用
        plt.plot(t_pre_range, self.D_pre_direct[0])  # 画出直接预测结果(上海外不用

...
class SIR:  # SIR传染病模型
    ...
    def function(self,initial_value,_):  # 方法：微分方程表达式（暂时不懂具体，不过先用着）
        ...
        y[0] = -(self.beta * x[0] * x[1]) / self.total_population  # 易感个体变化
        y[1] = (self.beta * x[0] * x[1]) / self.total_population - self.gamma * x[1]  # 感染个体变化
        y[2] = self.gamma * x[1]  # 治愈个体变化
    ...
    def algorithm(self):
        ...
        plt.plot(res[:, 1], color='red', label='Infection', marker='.')  # 画出感染者
        plt.plot(res[:, 2], color='green', label='Recovery', marker='.')  # 画出治愈者

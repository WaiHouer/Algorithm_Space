...
class SEAQURD:
    def __init__(self,region_num,file_name,start,end,total_population,S,E,A,Q,U,R,D,para,ksi,a):
        ...
        for i in range(len(self.region)):
            ...
            for t in range(1,self.T):
                ...
                for j in range(len(self.region)):
                    ...
                    a_rate += (self.A[j][t-1] - self.ksi * self.a[j]) / self.region[j].N * self.beta_a[j] * self.m[i][j]
                    u_rate += (self.U[j][t-1] - self.ksi * self.a[j]) / self.region[j].N * self.beta_u[j] * self.m[i][j]

...
class Epidemic_Model:  # 完整传染病模型
    def __init__(self,file_name):
        ...
        # for i in range(len(self.simulate_para_list)):
        #     print(f'仿真第{i + 1}天的参数列表：{self.simulate_para_list[i]}')
    ...
    def rolling_predict(self):
        ...
        for i in range(iteration):
            ...
            if i == 0:
                ...
                for b in range(rolling_num):
                    self.simulate_para_list[i + b] = self.final_para
                ...
                for b in range(rolling_num):  # 注意第一个衔接点不要覆盖成新参数
                    self.simulate_para_list[i * self.rolling_num + b] = self.final_para
